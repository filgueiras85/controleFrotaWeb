<%-- 
    Document   : exclusaoveiculo
    Created on : Feb 5, 2017, 3:29:35 PM
    Author     : ssdorneles
--%>
...
<html>
    ...
    <body>
        ...
        <p>&nbsp;</p>
        <p>&nbsp;</p>
        <div style="background-color:#eeeeee; background-position:initial initial; background-repeat:initial initial; border:1px solid #cccccc; padding:5px 10px; text-align:center">Exclua um veículo</div>


        <%
            VeiculoController veiculoController = new VeiculoController();
            List veiculos = veiculoController.listaTodosVeiculosAtivos();
            ArrayList placasArray = new ArrayList();
            for (Object o : veiculos) {
                Veiculo f = (Veiculo) o;
                placasArray.add(f.getPlaca());
            }

            pageContext.setAttribute("placasArray", placasArray);
        %>

        <br>

        <form action="exclusaoveiculo" method="post">

            <table align="center" style="border-spacing: 35px 2px">
                <tbody>
                    <tr>
                        <td>Qual veículo deseja excluir?</td>
                        <td><select id="dropdownUsername" name="placa">
                                <c:forEach items="${placasArray}" var="current">
            ...
            <p style="text-align:center"><input type="submit" value="Excluir Veículo" /></p>
        </form>
    </body>
</html>